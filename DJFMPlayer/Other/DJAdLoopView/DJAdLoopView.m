...
@interface DJAdLoopView ()<UIScrollViewDelegate>
{
...
@implementation DJAdLoopView

+ (instancetype)picViewWithLoadImageBlock:(LoadImageBlock)loadBlock{
    DJAdLoopView *adView = [[DJAdLoopView alloc] init];
...
- (void)jumpToLink:(UITapGestureRecognizer *)gester {
    ...
    id<DJAdLoopProtocol> adM = self.picModels[tag];
...
#pragma mark - setter
-(void)setPicModels:(NSArray<id <DJAdLoopProtocol>> *)picModels
{
    ...
    for (int i = 0; i< count; i++) {
        ...
        id<DJAdLoopProtocol> picM = picModels[i % baseCount];
        // 1. 创建控件
...
- (void)caculateCurrentPage: (UIScrollView *)scrollView {
    ...
    if (self.picModels.count == 1) {
        _currentPage = 1;
        if ([self.delegate respondsToSelector:@selector(adLoopViewDidSelectedPicModel:)]) {
            [self.delegate adLoopViewDidSelectedPicModel:self.picModels[self.pageControl.currentPage]];
        }
        return;
    }
    // 确认中间区域
