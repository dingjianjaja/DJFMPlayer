...
@interface DJSegmentBarVC ()<UIScrollViewDelegate,DJSegmentBarDelegate>

@property (nonatomic, weak) UIScrollView *contentView;

@end

@implementation DJSegmentBarVC
...
- (void)setUpWithItems:(NSArray<NSString *> *)items childVCs:(NSArray<UIViewController *> *)childVCs{
    NSAssert(items.count != 0 || items.count == childVCs.count,@"个数不一致");
...
- (DJSegmentBar *)segmentBar{
    if (!_segmentBar) {
        DJSegmentBar *segmentbar = [DJSegmentBar segmentBarWithFrame:CGRectZero];
